var extended_policies = function(req, res, policy, fn) {
  ...
  if (!_.isObject(obj[policy])) {
    ...
    sails.log.info("action", req.options.action);
    if (!_.isEmpty(req.options.action) && obj[policy].hasOwnProperty(req.options.action)) {
      var action = req.options.action;
      if (_.isBoolean(obj[policy][req.options.action]) && !obj[policy][req.options.action]) {
        ...
        sails.log.info("6");
        sails.log("needAuth[" + action + "]", obj[policy][action]);
