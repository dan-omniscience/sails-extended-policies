  ...
  if (!_.isObject(obj[policy])) {
    ...
    if (!_.isEmpty(req.options.action) && obj[policy].hasOwnProperty(req.options.action)) {
      ...
      if (_.isBoolean(obj[policy][action]) && !obj[policy][action]) {
        ...
        if (_.isArray(obj[policy][action])) {
          if (debug) {
            sails.log.debug("action is a multi policies (array)");
            sails.log.debug("================================");
          }
          async.map(
            obj[policy][action],
            function(pol, callback) {
              sails.hooks.policies.lookupFn(pol)(req, res, function(result) {
                return callback(null, result);
              });
            },
            function(err, results) {
              return fn(_.every(results));
            }
          );
        }
        // if not boolean and not string and not array deny all
        if (debug) {
      ...
      if (obj[policy].hasOwnProperty("*")) {
        ...
        if (_.isBoolean(obj[policy]["*"])) {
          ...
          if (_.isString(obj[policy]["*"])) {
            ...
            if (_.isArray(obj[policy]["*"])) {
              if (debug) {
                sails.log.debug("* is a multi policies (array)");
                sails.log.debug("================================");
              }
              async.map(
                obj[policy]["*"],
